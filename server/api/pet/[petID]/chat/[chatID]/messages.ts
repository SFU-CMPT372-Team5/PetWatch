...
export default defineEventHandler(async (event) => {
  ...
  if (token?.sub != undefined) {
    ...
    if (chatHost == undefined) {
      ...
      // Return an object with a 'status' property set to 401 and a message indicating the failure to authorize with the chat host.
      return { status: 401, message: "Failed to authorize with chat host" };
    }

    // Find all messages associated with the provided chat ID and sort them by the 'timeSent' property in ascending order.
  ...
  // Return an object with a 'status' property set to 401 and a message indicating that the user is not logged in.
